...
exports.iterateTimes1 = iterateTimes1;
...
function iterateTimes1(start, end, unit, timeSteps, callback) {
  var newUnit = "day";
  var time = (0, _moment["default"])(start).startOf(newUnit);

  if (timeSteps[newUnit] && timeSteps[newUnit] > 1) {
    var value = time.get(newUnit);
    time.set(newUnit, value - value % timeSteps[newUnit]);
  }

  while (time.valueOf() < end) {
    // console.log("LOG:: ", {time: time.valueOf(), end})
    var nextTime = (0, _moment["default"])(time).add(timeSteps[newUnit] || 1, "".concat(newUnit, "s"));
    callback(time, nextTime);
    time = nextTime;
  }
