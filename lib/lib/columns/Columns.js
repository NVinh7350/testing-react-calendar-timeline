...
function (_Component) {
  ...
  _createClass(Columns, [{
    ...
    value: function shouldComponentUpdate(nextProps) {
      console.log("RUN COLUMNS - shouldComponentUpdate:: ", {
        props: this.props,
        nextProps: nextProps
      });
      return !(nextProps.canvasTimeStart === this.props.canvasTimeStart && nextProps.canvasTimeEnd === this.props.canvasTimeEnd && // nextProps.canvasWidth === this.props.canvasWidth &&
      nextProps.lineCount === this.props.lineCount && nextProps.minUnit === this.props.minUnit && nextProps.timeSteps === this.props.timeSteps && // nextProps.height === this.props.height &&
    ...
    value: function render() {
      ...
      (0, _calendar.iterateTimes1)(canvasTimeStart, canvasTimeEnd, minUnit, timeSteps, function (time, nextTime) {
        var minUnitValue = time.get(minUnit === 'day' ? 'date' : minUnit);
        var firstOfType = minUnitValue === (minUnit === 'day' ? 1 : 0);
        var classNamesForTime = [];
        ...
        var left = getLeftOffsetFromDate(time.valueOf());
        var right = getLeftOffsetFromDate(nextTime.valueOf());
        newLines.push(_react["default"].createElement("div", {
          key: "line-".concat(time.valueOf()),
          className: classNames,
          style: {
            pointerEvents: 'none',
